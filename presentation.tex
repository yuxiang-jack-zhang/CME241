...
\author{Patrick Flanagan \and Jack Zhang \and Jeff}
\date{Winter 2026}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item For every state $(w_i, y, z)$: try all 165 actions, pick the best
        \item Store optimal value and policy
    \end{itemize}
\end{enumerate}
...
\vspace{0.2em}
{\scriptsize $^\dagger$ Hypothetical: 500-pt wealth grid, 10\% alloc.\ increments for Phase 3.}

\vspace{0.3em}
$\mathbf{119\times}$ \textbf{blowup} -- and this is still just 3 assets!
\end{column}
...
\vspace{1em}
\textbf{Solution:} Function approximation.\\
Learn $Q(s,a;\mathbf{w})$ from experience\\instead of tabulating every state.
...
\begin{itemize}
    \item Start: $\epsilon = 1.0$ (fully random -- explore the entire action space)
    \item Decay linearly over 8,000 episodes (Linear Q, SARSA, DQN) or 5,000 (PPO)
    \item End: $\epsilon = 0.05$ (mostly greedy, with 5\% exploration)
    \item \textbf{Why GLIE matters:} Guarantees every state-action pair is visited infinitely often (exploration) while the policy converges to greedy (exploitation). This is a \textit{necessary condition} for SARSA convergence (Theorem 12.4.1).
\end{itemize}

\vspace{0.3em}
\textbf{Financial intuition:} Early episodes are like a junior analyst trying random strategies; later episodes are a seasoned portfolio manager making targeted adjustments.
\end{frame}
...
\begin{enumerate}
    \item \textbf{Experience Replay Buffer} (500,000 transitions)
    \begin{itemize}
        \item Store $(s, a, r, s', \text{done})$ tuples as the agent interacts
        \item Sample random mini-batches (128) for training
        \item Breaks temporal correlation $\to$ more stable gradient updates
        \item Reuses data $\to$ more sample-efficient
    \end{itemize}

    \vspace{0.3em}
    \item \textbf{Target Network} (updated every 1,000 steps)
    \begin{itemize}
        \item Separate copy of the Q-network, $Q(s,a;\mathbf{w}^-)$, used in the TD target
        \item Target: $r + \gamma \max_{a'} Q(s', a'; \mathbf{w}^-)$ -- uses \textit{frozen} weights
        \item Prevents the ``moving target'' problem where the target shifts with every gradient step
    \end{itemize}
\end{enumerate}

\vspace{0.3em}
\textbf{Architecture (Dueling DQN):} Shared trunk (8 $\to$ 256 $\to$ 256, Tanh), then $\to$ Value stream (256 $\to$ 1) and Advantage stream (256 $\to$ 210). $Q = V + A - \bar{A}$. Total: $\sim$122K parameters.
\end{frame}
...
\]
where $A^{\pi}(s,a) = Q^{\pi}(s,a) - V^{\pi}(s)$ is the \textbf{advantage} (how much better action $a$ is than average).

\vspace{0.5em}
\textbf{PPO's clipped surrogate objective} (prevents destructively large updates):
\[
L(\theta) = \mathbb{E}\!\left[\min\!\left(\frac{\pi_\theta(a|s)}{\pi_{\theta_\text{old}}(a|s)} A,\;\; \text{clip}\!\left(\frac{\pi_\theta}{\pi_{\theta_\text{old}}}, 1\!-\!\epsilon, 1\!+\!\epsilon\right) A \right)\right]
\]

\vspace{0.3em}
\textbf{Actor-Critic architecture:}
\begin{itemize}
    \item \textbf{Actor} $\pi_\theta(a|s)$: outputs action probabilities (policy network)
    \item \textbf{Critic} $V_\phi(s)$: estimates state value (used to compute advantage via GAE)
    \item \textbf{Separate} networks (actor: $\sim$122K params, critic: $\sim$68K params). Total: $\sim$191K.
    \item Both use 256-256 hidden layers with Tanh and orthogonal initialization.
\end{itemize}
\end{frame}
...
\vspace{0.5em}
Both algorithms update $Q(S, A)$ using a TD target. The \textit{only} difference:

\vspace{0.3em}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\centering
\textbf{Q-Learning} (off-policy)\\[0.3em]
...
\begin{tikzpicture}[scale=0.85, every node/.style={font=\small}]
    \node[draw, rounded corners, fill=blue!15, minimum width=1.4cm, minimum height=0.6cm] (SA) at (0,0) {$S, A$};
    \node[draw, circle, fill=orange!20, minimum size=0.6cm] (R) at (0,-1.0) {$R$};
    \node[draw, ellipse, fill=blue!15, minimum width=1.4cm, minimum height=0.6cm] (Sp) at (0,-2.0) {$S'$};
    \node[draw, rounded corners, fill=green!20, minimum width=1.4cm, minimum height=0.6cm] (Ap) at (0,-3.0) {$A'$};
    \draw[->, thick] (SA) -- (R);
    \draw[->, thick] (R) -- (Sp);
    \draw[->, thick] (Sp) -- (Ap);
    \node[right, font=\small] at (1.0, 0) {Current state-action};
    \node[right, font=\small] at (1.0, -1.0) {Observe reward};
    \node[right, font=\small] at (1.0, -2.0) {Observe next state};
    \node[right, font=\small, text=green!40!black] at (1.0, -3.0) {Choose $A'$ via $\epsilon$-greedy $\leftarrow$ \textbf{same} $A'$ in update \& execution};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=1.0]
    % Axis
    \draw[->, thick] (0,0) -- (12,0);
    \node[below] at (0, -0.3) {$\lambda = 0$};
    \node[below] at (6, -0.3) {$\lambda \in (0,1)$};
    \node[below] at (12, -0.3) {$\lambda = 1$};
    ...
    % Our SARSA
    \draw[->, thick, green!50!black] (0, -1.2) -- (0, -0.4);
    \node[below, font=\bfseries, text=green!50!black] at (0, -1.3) {Our SARSA};
\end{tikzpicture}
\end{center}
...
\vspace{0.5em}
\textbf{Bias-variance tradeoff:} TD(0) has the lowest variance but highest bias. MC has zero bias but highest variance. In our 40-step episodes, TD(0) learns faster because variance dominates.
\end{frame}
...
SARSA converges to the optimal action-value function $Q(s,a) \to Q^*(s,a)$ under:
\begin{enumerate}
    \item \textbf{GLIE} schedule: $\epsilon_k \to 0$, but every $(s,a)$ visited infinitely often
    \item \textbf{Robbins-Monro} step sizes:
    \[
...
\textbf{Function approximation:} $Q(s, a; \mathbf{w}) = \mathbf{w}_a^\top \phi(s)$

\vspace{0.2em}
Feature vector with polynomial and interaction terms:
\[
\phi(s) = \bigl[\;\underbrace{s_{1:8}}_{\text{8 raw}},\;\; \underbrace{t^2, w^2, tw}_{\text{3 poly}},\;\; \underbrace{t \cdot \text{onehot}(y),\; t \cdot \text{onehot}(z)}_{\text{6 time-regime crosses}},\;\; \underbrace{w \cdot \text{onehot}(y),\; w \cdot \text{onehot}(z)}_{\text{6 wealth-regime crosses}},\;\; \underbrace{1}_{\text{bias}}\;\bigr] \in \mathbb{R}^{24}
\]

Separate $\mathbf{w}_a \in \mathbb{R}^{24}$ for each of 210 actions. Total parameters: $210 \times 24 = 5{,}040$.
...
def _expand_features(self, state):
    ...
    poly = [t**2, w**2, t*w]
    return np.concatenate([state, poly, t*cat, w*cat, [1.0]])  # dim=24
...
def update(self, state, action, reward, next_state, done):
    ...
    if not done:
        ...
        td_err = np.clip(reward + q_next - q_sa, -10, 10)  # clip for stability
    else:
...
\vspace{0.5em}
\textbf{Consumption Sharpe ratio:}
\[
\text{C-Sharpe} = \frac{\text{mean}(\bar{c}_t)}{\text{std}(\bar{c}_t)}
\]
Higher = smoother consumption over time.
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Key takeaways:}

\vspace{0.5em}
All RL agents have \textbf{C-Sharpe $> 10$}, vs.\ $\sim$2.7 for baselines.

$\Rightarrow$ RL consumption is $4$--$10\times$ smoother.

\vspace{0.5em}
\textbf{Max drawdown} is higher for RL (54--66\%):
\begin{itemize}
    \item RL agents consume more $\to$ less savings buffer
    \item Higher drawdown is an \textit{accepted trade-off} for better utility
\end{itemize}

\vspace{0.5em}
\textbf{Linear Q has the highest Min $W_T$} among RL agents (\$33.2):
\begin{itemize}
    \item Rich feature expansion $\to$ learns cautious low-wealth behavior
    \item SARSA (\$24.9) is 2nd; on-policy avoids overconfident actions
\end{itemize}

\vspace{0.3em}
\textbf{No ruin in any strategy} -- $P(W_T < 10) = 0\%$.
\end{column}
\end{columns}
\end{frame}
...
% ============================================================
% SLIDE 21: Agent Comparison Summary
% ============================================================
