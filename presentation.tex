...
\begin{itemize}
    ...
    \item \textbf{Why GLIE matters:} Every $(s,a)$ visited infinitely often while policy converges to greedy -- \textit{necessary condition} for SARSA convergence
\end{itemize}
...
% SLIDE 12: Linear Q
% ============================================================
\begin{frame}{Linear Q-Learning}

\textbf{Key idea:} $Q(s,a;\mathbf{w}) = \mathbf{w}_a^\top \phi(s)$ -- linear approximation with Q-learning updates ($210 \times 24 = 5{,}040$ params).
...
\vspace{0.2em}
\textbf{Off-policy TD update:}
\[
\Delta \mathbf{w}_a = \alpha \underbrace{\bigl(R + \gamma \cdot \max_{a'} Q(S', a'; \mathbf{w}) - Q(S, A; \mathbf{w})\bigr)}_{\text{TD error, clipped at } \pm 10} \cdot \phi(S)
\]
...
\textbf{Advantages:}
\begin{itemize}
    \item Tiny and fast ($\sim$18s to train)
    \item Interpretable weights per action
    \item Off-policy $\Rightarrow$ aggressive, sample-efficient learning
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Limitations:}
\begin{itemize}
    \item Linear -- misses complex nonlinear structure
    \item Deadly triad: FA + bootstrap + off-policy
    \item Requires hand-crafted features
\end{itemize}
\end{column}
\end{columns}

\vspace{0.2em}
\textbf{Result:} $\mathbb{E}[\text{Utility}] = -1.59$ (second best overall, behind DQN at $-1.54$).
\end{frame}
% ============================================================
% SLIDE 13: SARSA
% ============================================================
...
\textbf{Same as Linear Q} (features, params, $\epsilon$-greedy). \textit{One difference} in the TD target -- use $A'$ \textit{actually taken}:

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\centering
\textbf{Linear Q} (off-policy)\\[0.2em]
\fbox{$\Delta \mathbf{w} = \alpha \bigl(R + \gamma \cdot \underset{a}{\max}\, Q(S'\!, a) - Q(S,A)\bigr) \nabla Q$}
\vspace{0.15em}
``What's the \textit{best} I \textit{could} do?''
\end{column}
\begin{column}{0.48\textwidth}
\centering
...
\vspace{0.15em}
``What will I \textit{actually} do?''
\end{column}
\end{columns}

\vspace{0.2em}
\textbf{Why on-policy matters:}
\begin{itemize}
    \item Accounts for exploration cost -- never overestimates Q-values from random actions
    \item \textbf{Safest} theoretical combination: linear FA + on-policy avoids the deadly triad
    \item Convergence to $Q^*$ guaranteed (tabular, GLIE + Robbins-Monro); stable in practice
    \item Key detail: $A'$ from update \textit{must} be cached and reused as the next executed action
\end{itemize}

\vspace{0.15em}
$\Rightarrow$ More conservative than Linear Q ($-1.88$ vs $-1.59$), but \textbf{safest policy} (Min $W_T = \$24.9$) and most theoretically grounded.
\end{frame}
% ============================================================
...
\vspace{2em}
\normalsize
Repository: \texttt{github.com/yuxiang-jack-zhang/CME241}
\end{frame}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 1 -- Title}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 2 -- Outline}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 3 -- The Lifecycle Allocation Problem}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 4 -- MDP Formulation}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 5 -- Transition Dynamics}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 6 -- Phase 2: Exact DP Solution}
\textbf{[Jack speaks -- 1.5 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 7 -- Curse of Dimensionality}
\textbf{[Jeffrey speaks -- 1 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 8 -- Phase 3: RL Architecture}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 9 -- Exploration--Exploitation}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 10 -- Deep Q-Network (DQN)}
\textbf{[Jack speaks -- 1.5 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 11 -- Proximal Policy Optimization (PPO)}
\textbf{[Jack speaks -- 1.5 min]}\\
``PPO takes a completely different approach from the value-based methods. Instead of learning Q-values and deriving the policy, PPO directly parameterizes the policy as a neural network and optimizes it using the policy gradient theorem.''\\[0.5em]
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 12 -- Linear Q-Learning}
\textbf{[Patrick speaks -- 1 min]}\\
...
``The update rule is standard Q-learning: observe a transition, compute the TD error as the Bellman residual using the greedy next action, and take a gradient step. Because it's off-policy -- it bootstraps from the max Q-value regardless of what the agent will actually do -- it learns more aggressively than SARSA. This is why Linear Q achieves better utility: -1.59 versus SARSA's -1.88.''\\[0.5em]
``The trade-off is theoretical: off-policy + function approximation + bootstrapping is the deadly triad, which can cause divergence. In practice it's stable here because the linear model is well-behaved and we clip TD errors at plus or minus 10.''
\end{frame}

\begin{frame}[allowframebreaks]{Speaker Notes: Slide 13 -- SARSA: On-Policy TD Control}
\textbf{[Patrick speaks -- 1.5 min]}\\
``SARSA has exactly the same architecture as Linear Q -- same features, same 5,040 parameters, same epsilon-greedy schedule. The only difference is one word in the update: instead of using max over Q at the next state, we use Q of S-prime, A-prime, where A-prime is the action the agent will actually take next, including random exploration.''\\[0.5em]
``This makes SARSA on-policy. It's asking: given that I'll sometimes explore randomly, what's my realistic expected future value? Q-learning is optimistic -- it assumes future actions are always greedy. SARSA is honest about the cost of exploration.''\\[0.5em]
``The practical consequence: SARSA is more conservative. In a financial setting, this is arguably desirable -- you don't want a policy calibrated assuming you'll always act optimally. The result: utility of -1.88 versus -1.59 for Linear Q, but SARSA has the best minimum terminal wealth at 24.9 -- it never assumes it will make optimal decisions in bad states.''\\[0.5em]
``The theoretical consequence: linear FA plus on-policy avoids the deadly triad, making SARSA the most theoretically grounded of our four agents. Convergence to optimal Q is guaranteed in the tabular case under GLIE and Robbins-Monro step sizes. The key implementation detail is caching A-prime during the update so the same action is actually executed next step -- without this you silently break on-policy consistency.''
\end{frame}

\begin{frame}[allowframebreaks]{Speaker Notes: Slide 16 -- Training Convergence}
\textbf{[Jeffrey speaks -- 1 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 17 -- Learned Policy Heatmaps}
\textbf{[Jeffrey speaks -- 1 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 18 -- Monte Carlo Comparison}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 19 -- Risk Metrics}
\textbf{[Jeffrey speaks -- 1 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 20 -- Wealth Trajectories}
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 21 -- Utility Distributions}
\textbf{[Jeffrey speaks -- 45 sec]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 22 -- Comparing Agents}
\textbf{[Jack speaks -- 1 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 23 -- Thank You \& Questions}
\textbf{[Patrick speaks -- 1 min]}\\
...
\begin{frame}[allowframebreaks]{Speaker Notes: Slide 24 -- Slide 24}
...
\textit{``Is SARSA convergence guaranteed here?''} -- Not with function approximation. The convergence theorem is for tabular. But linear FA + on-policy is the safest combination -- avoids the deadly triad of FA + bootstrapping + off-policy.\\[0.3em]
